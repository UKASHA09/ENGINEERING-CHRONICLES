...
// Event listener for the input field to trigger the search and display suggestions
document.getElementById("material").addEventListener("input", showMaterialSuggestions);

// Function to show material suggestions based on user input
function showMaterialSuggestions() {
  var input = document.getElementById("material").value;
  var suggestions = getMatchingMaterials(input);
  displayMaterialSuggestions(suggestions);
}

// Function to get materials matching the user input
function getMatchingMaterials(input) {
  input = input.toLowerCase();
  return availableMaterials.filter(function(material) {
    return material.toLowerCase().startsWith(input);
  });
}

// Function to display material suggestions
function displayMaterialSuggestions(suggestions) {
  var suggestionsList = document.getElementById("suggestions");
  suggestionsList.innerHTML = ""; // Clear previous suggestions

  suggestions.forEach(function(material) {
...
// Function to select a material from the suggestions and display its properties
function selectMaterial(material) {
  var properties = getMaterialProperties(material);
  displayMaterialProperties(material, properties);
}

// Function to get properties of a material
function getMaterialProperties(material) {
  var properties = {};

  // Add properties for all materials here
  if (material === "alloy") {
